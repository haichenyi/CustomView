...
public class PickValueView extends LinearLayout implements NumberPicker.OnValueChangeListener {
    ...
    private void generateView() {
        ...
        for (TextView title : titles) {
            ...
            title.setTextColor(Color.parseColor("#3434EE"));
        }
        ...
        for (MyNumberPicker np : nps) {
            np.setLayoutParams(params);
            np.setDescendantFocusability(FOCUS_BLOCK_DESCENDANTS);
    ...
    private void initViewAndPicker() {
        ...
        if (mLeftValues != null && mLeftValues.length != 0) {
            if (mLeftValues.length < DATA_SIZE) {
                System.arraycopy(mLeftValues, 0, mShowingLeft, 0, mLeftValues.length);
                for (int i = mLeftValues.length; i < DATA_SIZE; i++) {
                    mShowingLeft[i] = -9999;
                }
            } else {
                System.arraycopy(mLeftValues, 0, mShowingLeft, 0, DATA_SIZE);
            }
        ...
        if (mViewCount == 2 || mViewCount == 3) {
            if (mMiddleValues != null && mMiddleValues.length != 0) {
                if (mMiddleValues.length < DATA_SIZE) {
                    System.arraycopy(mMiddleValues, 0, mShowingMiddle, 0, mMiddleValues.length);
                    for (int i = mMiddleValues.length; i < DATA_SIZE; i++) {
                        mShowingMiddle[i] = -9999;
                    }
                } else {
                    System.arraycopy(mMiddleValues, 0, mShowingMiddle, 0, DATA_SIZE);
                }
        ...
        if (mViewCount == 3) {
            if (mRightValues != null && mRightValues.length != 0) {
                if (mRightValues.length < DATA_SIZE) {
                    System.arraycopy(mRightValues, 0, mShowingRight, 0, mRightValues.length);
                    for (int i = mRightValues.length; i < DATA_SIZE; i++) {
                        mShowingRight[i] = -9999;
                    }
                } else {
                    System.arraycopy(mRightValues, 0, mShowingRight, 0, DATA_SIZE);
                }
     ...
     * 设置数据--单列数据
     *
     * @param leftValues        leftValues
     * @param mDefaultLeftValue mDefaultLeftValue
     */
     ...
     * @param leftValues         leftValues
     * @param mDefaultLeftValue  mDefaultLeftValue
     * @param middleValues       middleValues
     * @param defaultMiddleValue defaultMiddleValue
     */
     ...
     * @param middleValues       middleValues
     * @param defaultMiddleValue defaultMiddleValue
     * @param rightValues        rightValues
     * @param defaultRightValue  defaultRightValue
     */
